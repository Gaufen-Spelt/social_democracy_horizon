...
/* =====================================================================
   SAVE/LOAD — PAGINATED SLOTS (20 + 20)
   Generates numbered slot rows 0-19 (page 1) and 20-39 (page 2) using
   the same id convention and dendryUI hooks the original static rows
   used (save_button_N / delete_button_N / save_info_N / export_button_N),
   so the base engine's existing save/load/delete/export logic in
   core.js picks them up with no changes there.
   ===================================================================== */

window.SAVE_SLOTS_PER_PAGE = 20;
window.SAVE_PAGE_COUNT = 2; // 40 total numbered slots

window._buildSaveSlotRow = function(n) {
  return (
    '<tr>' +
      '<td>' + n + '</td>' +
      '<td><button class="b save_button" id="save_button_' + n + '">Save</button></td>' +
      '<td><button class="b delete_button" id="delete_button_' + n + '">Delete</button></td>' +
      '<td class="save_info" id="save_info_' + n + '"></td>' +
      '<td><button class="b export_button" id="export_button_' + n + '">Export</button></td>' +
    '</tr>'
  );
};

window._populateSaveSlotPages = function() {
  for (var page = 1; page <= window.SAVE_PAGE_COUNT; page++) {
    var tableEl = document.getElementById('saves_table_page_' + page);
    if (!tableEl || tableEl.dataset.populated === '1') continue;

    var startSlot = (page - 1) * window.SAVE_SLOTS_PER_PAGE;
    var endSlot = startSlot + window.SAVE_SLOTS_PER_PAGE - 1;
    var html = '';
    for (var n = startSlot; n <= endSlot; n++) {
      html += window._buildSaveSlotRow(n);
    }
    tableEl.innerHTML = html;
    tableEl.dataset.populated = '1';

    // Wire each generated button to the same dendryUI handlers the
    // original static buttons used (mirrors core.js's own wiring
    // convention: it binds by id, so we replicate that here).
    for (var i = startSlot; i <= endSlot; i++) {
      (function(slot) {
        var saveBtn = document.getElementById('save_button_' + slot);
        var deleteBtn = document.getElementById('delete_button_' + slot);
        var exportBtn = document.getElementById('export_button_' + slot);
        if (saveBtn) saveBtn.onclick = function() { dendryUI.saveGame(slot); };
        if (deleteBtn) deleteBtn.onclick = function() { dendryUI.deleteGame(slot); };
        if (exportBtn) exportBtn.onclick = function() { dendryUI.exportSave(slot); };
      })(i);
    }
  }

  if (window.dendryUI && typeof window.dendryUI.refreshSaveSlots === 'function') {
    window.dendryUI.refreshSaveSlots();
  } else if (window.dendryUI && typeof window.dendryUI.showSaveSlots === 'function') {
    // no-op: showSaveSlots is what triggered this in the first place
  }
};

window.showSavePage = function(page) {
  window._populateSaveSlotPages();

  for (var p = 1; p <= window.SAVE_PAGE_COUNT; p++) {
    var tableEl = document.getElementById('saves_table_page_' + p);
    var tabEl = document.getElementById('save_page_tab_' + p);
    if (tableEl) tableEl.style.display = (p === page) ? '' : 'none';
    if (tabEl) tabEl.className = tabEl.className.replace(' active', '') + (p === page ? ' active' : '');
  }
};

var _saveSlotsPatchInterval = setInterval(function() {
  if (!window.dendryUI || typeof window.dendryUI.showSaveSlots !== 'function') return;
  clearInterval(_saveSlotsPatchInterval);

  var _originalShowSaveSlots = window.dendryUI.showSaveSlots.bind(window.dendryUI);
  window.dendryUI.showSaveSlots = function() {
    _originalShowSaveSlots();
    window._populateSaveSlotPages();
    window.showSavePage(1);
  };
}, 100);
